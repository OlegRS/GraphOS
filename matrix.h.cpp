...
template <typename T> matrix<T>::matrix(const std::string &file_name) {  
  ...
  if( !ifs.is_open() ) {
    std::cerr << "-------------------------------------------------------------------\n";
    std::cerr << "ERROR: Cannot open the file " << file_name << " to load the matrix!\n";
    std::cerr << "-------------------------------------------------------------------\n";
    exit(1);
  }
  ...
  if(dim_x % dim_y) {
    std::cerr << "--------------------------------------------------\n";
    std::cerr << "ERROR: Matrix " << file_name << " has uneven rows!\n";
    std::cerr << "--------------------------------------------------\n";
    exit(2);
  }
...
template <typename T> matrix<T>& matrix<T>::operator+=(const matrix<T> &M) {
  if(dim_x == M.dim_x && dim_y == M.dim_y) {
    ...
    std::cerr << "-------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Addition assignment is called on matrix with different dimensions!\n";
    std::cerr << "-------------------------------------------------------------------------\n";
    return *this;
  }
}
...
template <typename T> matrix<T>& matrix<T>::operator/=(double x) {
  if(x) {
    ...
    std::cerr << "----------------------------------------------\n";
    std::cerr << "ERROR: Elementwise division of matrix by zero!\n";
    std::cerr << "----------------------------------------------\n";
    exit(1);
    //return *this;
  }
...
template <typename T> matrix<T> matrix<T>::max_n_entries(const unsigned int &n) const {
  ...
  if(n <= sz) {
    ...
    std::cerr << "---------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to find max "<< n <<" entries of matrix with only " << sz << " elements!\n";
    std::cerr << "---------------------------------------------------------------------------------------\n";
    return matrix<T>();
  }
}
...
unsigned int sz = dim_x*dim_y;
  if(n <= sz) {
    ...
    std::cerr << "-----------------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to keep the largest "<< n <<" entries of matrix with only " << sz << " elements!\n";
    std::cerr << "-----------------------------------------------------------------------------------------------\n";
  }
  return *this;
}
...
template <typename T> matrix<T> matrix<T>::min_n_entries(const unsigned int &n) const {
  ...
  if(n <= sz) {
    ...
    std::cerr << "---------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to find min "<< n <<" entries of matrix with only " << sz << " elements!\n";
    std::cerr << "---------------------------------------------------------------------------------------\n";
    return matrix<T>();
  }
}
...
unsigned int sz = dim_x*dim_y;
  if(n <= sz) {
    ...
    std::cerr << "------------------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to keep the smallest "<< n <<" entries of matrix with only " << sz << " elements!\n";
    std::cerr << "------------------------------------------------------------------------------------------------\n";
  }
  return *this;
}
...
template <typename T> double root_mean_square(const matrix<T> &M1, const matrix<T> &M2) {
  ...
  if(sz == M2.dim_x*M2.dim_y) {
    ...
    std::cerr << "--------------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to compute Euclidean distance of matrices with defferent numbers of elements!\n";
    std::cerr << "--------------------------------------------------------------------------------------------\n";
    return 0;
  }
}
...
template <typename T> unsigned int Hamming_distance(const matrix<T> &M1, const matrix<T> &M2) {
  ...
  if(sz == M2.dim_x*M2.dim_y) {
    ...
    std::cerr << "------------------------------------------------------------------------------------------\n";
    std::cerr << "ERROR: Attempt to compute Hamming distance of matrices with defferent numbers of elements!\n";
    std::cerr << "------------------------------------------------------------------------------------------\n";
    return 0;
  }
}
