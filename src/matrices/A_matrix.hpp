...
class A_matrix : public symm_matrix<bool> {//Adjacency matrix
  ...
  A_matrix& sample_Markov_ERG(const unsigned int &N_iters, prng& rnd, const col_vector<double> &T, const double theta, const unsigned int &N_pairs_max=1, const bool &initialize_randomly=true, const double &temp=1); // Samples general homogeneous Markov Exponential Random Graph
