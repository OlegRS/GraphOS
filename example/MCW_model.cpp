...
int main() {
  ...
  S.save("./data/example_spin_sequence");
  // Can load the given spin sequence to the graph
  gr.set_spin_sequence(S);
  // Can save the graph to file
  gr.save("./data/example");
  // Can load the graph from file
  gr.clear();
  gr.load("./data/example.graph");

  ///////////// RUNNUNG IN A LOOP, SAVING TO FILE //////////////
  std::ofstream ofs("./data/example_file.csv");
  col_vector<double> magnetisations(Np.size());
  for(double H=-10; H<10; H+=0.5) {
    F[0]=F[1]=F[2]=H;
    gr.update_MCW_model(Np, J, F);
    // If you want to start every time from random configuration do
    gr.randomize_spins(rand());
    gr.simulate_Glauber_Dynamics_with_fields(rand(), temperature, N_iterations);
  ...
  ofs.close();
  // Now you can load example_file.csv to MATLAB, Python or spreadsheet
